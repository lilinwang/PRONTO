...
class series_mr_model extends CI_Model{
	...
	private function backup($data){
		$sql = 'INSERT INTO `series_mr_backup` VALUES(?';					
		$count = count($data);	
	...
	function delete_all(){
		...
		$sql = 'DELETE FROM series_mr WHERE 1';
		$query = $this->db->query($sql);		
	}	
	...
	function delete_one($id,$protocol_number){
		$sql = 'DELETE FROM series_mr WHERE `Series`=? and `Protocol ID`=?';
